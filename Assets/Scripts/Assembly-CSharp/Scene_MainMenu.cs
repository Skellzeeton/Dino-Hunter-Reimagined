...
public class Scene_MainMenu : MonoBehaviour
{
	...
	public void TUIEvent_SetUIInfo(object sender, TUIEvent.BackEvent_SceneMainMenu m_event)
	{
		...
		else if (m_event.GetEventName() == TUIEvent.SceneMainMenuEventType.TUIEvent_EnterInfo)
		{
			if (m_event.GetEventInfo() != null && m_event.GetEventInfo().villiage_enter_info != null)
			{
				...
				{
					go_camp_new.gameObject.SetActiveRecursive(true);
					go_camp_mark.gameObject.SetActiveRecursive(false);
				}
				else if (villiage_enter_info.equip_sign == NewMarkType.Mark)
				{
					go_camp_new.gameObject.SetActiveRecursive(false);
					go_camp_mark.gameObject.SetActiveRecursive(true);
				}
				else
				{
					go_camp_new.gameObject.SetActiveRecursive(false);
				...
				{
					go_skill_new.gameObject.SetActiveRecursive(true);
					go_skill_mark.gameObject.SetActiveRecursive(false);
				}
				else if (villiage_enter_info.skill_sign == NewMarkType.Mark)
				{
					go_skill_new.gameObject.SetActiveRecursive(false);
					go_skill_mark.gameObject.SetActiveRecursive(true);
				}
				else
				{
					go_skill_new.gameObject.SetActiveRecursive(false);
				...
				{
					go_tavern_new.gameObject.SetActiveRecursive(true);
					go_tavern_mark.gameObject.SetActiveRecursive(false);
				}
				else if (villiage_enter_info.tavern_sign == NewMarkType.Mark)
				{
					go_tavern_new.gameObject.SetActiveRecursive(false);
					go_tavern_mark.gameObject.SetActiveRecursive(true);
				}
				else
				{
					go_tavern_new.gameObject.SetActiveRecursive(false);
				...
				{
					go_stash_new.gameObject.SetActiveRecursive(true);
					go_stash_mark.gameObject.SetActiveRecursive(false);
				}
				else if (villiage_enter_info.stash_sign == NewMarkType.Mark)
				{
					go_stash_new.gameObject.SetActiveRecursive(false);
					go_stash_mark.gameObject.SetActiveRecursive(true);
				}
				else
				{
					go_stash_new.gameObject.SetActiveRecursive(false);
				...
				{
					go_blackmarket_new.gameObject.SetActiveRecursive(true);
					go_blackmarket_mark.gameObject.SetActiveRecursive(false);
				}
				else if (villiage_enter_info.blackmarket_sign == NewMarkType.Mark)
				{
					go_blackmarket_new.gameObject.SetActiveRecursive(false);
					go_blackmarket_mark.gameObject.SetActiveRecursive(true);
				}
				else
				{
					go_blackmarket_new.gameObject.SetActiveRecursive(false);
		...
		else if (m_event.GetEventName() == TUIEvent.SceneMainMenuEventType.TUIEvent_SaleInfo)
		{
			...
			if (all_sale_info != null && all_sale_info.all_sale_info != null && all_sale_info.all_sale_info.Count > 0)
			{
				if (btn_sale != null)
				{
					btn_sale.gameObject.SetActiveRecursive(true);
					btn_sale.Reset();
				}
				OpenSaleBlink(true);
			}
			...
			{
				btn_sale.gameObject.SetActiveRecursive(false);
			}
		}
		...
		else if (m_event.GetEventName() == TUIEvent.SceneMainMenuEventType.TUIEvent_EnterEndless)
		{
			if (m_event.GetControlSuccess())
			{
				DoSceneChange(m_event.GetWparam(), "Scene_EndlessMainMenu");
				return;
			}
			m_fade_in_time = 0f;
			do_fade_in = false;
	...
	private void OpenMapBlink(bool m_open)
	{
		if (m_open)
		{
			...
			{
				img_map_bg.gameObject.SetActiveRecursive(true);
		...
		{
			img_map_bg.gameObject.SetActiveRecursive(false);
		}
	}
	...
	private void OpenCoopBlink(bool m_open)
	{
		if (m_open)
		{
			...
			{
				img_coop_bg.gameObject.SetActiveRecursive(true);
		...
		{
			img_coop_bg.gameObject.SetActiveRecursive(false);
		}
	}
	...
	private void OpenAchievementBlink(bool m_open)
	{
		if (m_open)
		{
			...
			{
				effect_achievement_bg.SetActiveRecursive(true);
			}
		}
		else if (effect_achievement_bg != null)
		{
			effect_achievement_bg.SetActiveRecursive(false);
		}
	}
	...
	private void OpenSaleBlink(bool m_open)
	{
		if (m_open)
		{
			...
			{
				effect_sale_bg.SetActiveRecursive(true);
			}
		}
		else if (effect_sale_bg != null)
		{
			effect_sale_bg.SetActiveRecursive(false);
		}
	}
	...
	private void OpenDailyMissionsBlink(bool m_open)
	{
		if (m_open)
		{
			...
			{
				effect_daily_missions_bg.SetActiveRecursive(true);
			}
		}
		else if (effect_daily_missions_bg != null)
		{
			effect_daily_missions_bg.SetActiveRecursive(false);
		}
	}
	...
	private void DoNewHelp()
	{
		...
		if (newHelpState != NewHelpState.Help_Over && newHelpState != NewHelpState.None)
		{
			...
			if (btn_coop != null)
			{
				btn_coop.gameObject.SetActiveRecursive(false);
			}

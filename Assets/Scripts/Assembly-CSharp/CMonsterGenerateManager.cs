...
public class CMonsterGenerateManager
{
        ...
        if (taskInfo != null && taskInfo.nType == 3)
        {
            ...
            if (pathWalkerManager != null)
            {
                ...
                foreach (WaveMobInfo item in waveInfo.m_ltWaveMobInfo)
                {
                    ...
                    if (!list.Contains(item.nStartPoint))
                ...
                {
                    var data = sPManagerGround.GetData();
                    var data2 = tPManagerEnd.GetData();
                    foreach (var item3 in data)
                    {
                        if (list.Count > 0 && !list.Contains(item3.Key))
                            continue;

                        foreach (var value in data2.Values)
                        {
    ...
    public void MGStart(int nWaveID)
    {
        ...
        if (cTaskDefence != null)
        {
            ...
            if (gameUI != null)
            {
                gameUI.ShowTip("YOU'RE ON WAVE " + cTaskDefence.m_nCurWave);
            }
        }
    }

...
public class Scene_CoopRoom : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (btn_enter != null)
		{
			btn_enter.gameObject.SetActiveRecursive(false);
		}
		if (btn_exit != null)
		{
			btn_exit.gameObject.SetActiveRecursive(false);
		}
	}
	...
	public void TUIEvent_SetUIInfo(object sender, TUIEvent.BackEvent_SceneCoopRoom m_event)
	{
		...
		else if (m_event.GetEventName() == TUIEvent.SceneCoopRoomEventType.TUIEvent_ShowBtnStart)
		{
			if (btn_start != null)
			{
				btn_start.gameObject.SetActiveRecursive(m_event.GetControlSuccess());
				if (m_event.GetControlSuccess())
				{
					btn_start.Reset();
				}

...
public class iGameUIBase : MonoBehaviour
{
	...
	public void AddGoldUI(float fValue, Vector3 v3Pos)
	{
		...
		if (!(gameObject == null))
		{
			...
			if (!(component == null))
			{
				...
				component.SetColor(new Color(1f, 1f, 0f));
				component.Go(v3Pos);
			}
		}
	}
	...
	public void ShowMaterial(bool bShow)
	{
		...
		{
			m_UIManager.mPanelMaterial.mBackButton.SetActiveRecursive(false);
		}
	}
	...
	public void ShowPauseUI(bool bShow)
	{
		...
		m_UIManager.mGamePauseDialog.gameObject.SetActiveRecursive(bShow);
	...
	public void ShowStashFullDialog(bool bShow, int nPrice = 0, int nCur = 1, int nNext = 1)
	{
		if (!(m_UIManager == null) && !(m_UIManager.mStashFullDialog == null))
		{
			...
			if (bShow)
			{
				...
				m_UIManager.mStashFullDialog.SetContent("Expand Stash Capacity to: " + nNext + "([11FF00]+ " + (nNext - nCur) + "[-])");
	...
	public void ShowGameUI()
	{
		...
		{
			m_UIManager.mWheelMove.gameObject.SetActiveRecursive(true);
		}
		...
		{
			m_UIAimCross.gameObject.SetActiveRecursive(true);
		}
		if (m_UIManager.mPause != null && !m_GameScene.isTutorialStage)
		{
			m_UIManager.mPause.gameObject.SetActiveRecursive(true);
		}
		ShowShootUI(true);
		foreach (gyUIPortrait value in m_dictTeamMate.Values)
		{
	...
	public void HideGameUI()
	{
		...
		if (m_UIManager.mFastWeapon != null)
		{
			m_UIManager.mFastWeapon.SetActiveRecursive(false);
		}
		if (m_UIManager.mSkill != null)
		{
			m_UIManager.mSkill.gameObject.SetActiveRecursive(false);
		}
		if (m_UIManager.mWheelMove != null)
		{
			m_UIManager.mWheelMove.gameObject.SetActiveRecursive(false);
		}
		if (m_UIManager.mWheelShoot != null)
		{
		...
		if (m_UIAimCross != null)
		{
			m_UIAimCross.gameObject.SetActiveRecursive(false);
		}
		if (m_UIManager.mPause != null)
		{
			m_UIManager.mPause.gameObject.SetActiveRecursive(false);
		}
		ShowPauseUI(false);
		ShowShootUI(false);
		foreach (gyUIPortrait value in m_dictTeamMate.Values)
	...
	public void ShowSkipUI(bool bShow)
	{
		...
		{
			m_UIManager.mSkip.gameObject.SetActiveRecursive(bShow);
		}
	}
	...
	public void ShowAimCross(bool bShow)
	{
		if (!(m_UIAimCross == null))
		{
			m_UIAimCross.gameObject.SetActiveRecursive(bShow);
		}
	}
	...
	{
		CUISound.GetInstance().Play("UI_Button");
		ShowMessageBox("Leave & lose rewards?", gyUIMessageBox.kMessageBoxType.OKCANCEL, CallBack_GoToVillage);
	}
	...
	{
		CUISound.GetInstance().Play("UI_Button");
		ShowMessageBox("Leave & lose rewards?", gyUIMessageBox.kMessageBoxType.OKCANCEL, CallBack_GoToMap);
	}
	...
	{
		CUISound.GetInstance().Play("UI_Button");
		ShowMessageBox("Leave & lose rewards?", gyUIMessageBox.kMessageBoxType.OKCANCEL, CallBack_GoToVillage_MutiplyEscape);
	}

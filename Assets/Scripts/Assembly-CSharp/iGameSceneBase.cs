...
public class iGameSceneBase
{
	...
	public virtual void Destroy()
	{
		...
		if (m_ltScreenTipTriggerEnd != null)
		{
			foreach (CPointScreenTip item in m_ltScreenTipTriggerEnd)
			{
				...
				{
					item.screentip.gameObject.SetActiveRecursive(false);
				}
			}
	...
	public virtual void StartGame()
	{
		...
		if (!CurGameLevelInfo.m_bLimitMelee)
		{
			...
			/*	if (!CurGameLevelInfo.m_bLimitRifle)
				{
					for (int j = 0; a < 3; j++)
					{
						CWeaponBase weapon = m_GameState.GetWeapon(j);
					if (weapon != null && weapon.CurWeaponLvlInfo != null && weapon.CurWeaponLvlInfo.nType != 2)
						{
							m_User.SwitchWeapon(j);
							flag = true;
							break;
				...
				if (!m_User.isDead)
				{
					if (m_GameData.m_HunterLevelCenter != null && m_curHunterLevelInfo != null)
					{
						...
						if (cHunterLevelInfo != null)
						{
							...
							m_DataCenter.AddGold(m_GameState.m_nLevelRewardGold + m_GameState.GainGoldInGame);
	...
	public virtual void ReviveGame()
	{
		...
		{
			m_GameUI.UIManager.mWheelMove.gameObject.SetActiveRecursive(true);
		}
		if (m_GameUI.UIManager.mWheelShoot != null)
		{
			m_GameUI.UIManager.mWheelShoot.gameObject.SetActiveRecursive(true);
		}
	}
	...
	protected virtual void UpdateStatus_GameOver(float deltaTime)
	{
		...
		if (m_GameUI.UIManager.mPanelMissionSuccessMutiply.m_Champion != null)
		{
			...
			m_GameUI.UIManager.mPanelMissionSuccessMutiply.m_Champion.SetActiveRecursive(true);
	...
	public void AddBulletTrack(Vector3 v3Src, Vector3 v3Dst, int nPrefab)
	{
		...
		if (!(poolObject == null))
		{
			...
			if (!(component == null))
			{
				...
				poolObject.SetActiveRecursive(true);
			}
		}
	}
	...
	public void AddDamageText(float fDamage, Vector3 v3Pos, bool bCritical = false)
	{
		if (!(m_GameUI == null))
		{
			...
			else
			{
				...
				CUISound.GetInstance().Play("Crit");
			}
		}
	}
	...
	public void InitTask(int nTaskID)
	{
		...
		switch (taskInfo.nType)
		{
			case 1:
			{
						...
						for (int i = 0; i < item.nMaxCount; i++)
						{
							...
							if (flag)
							{
								...
								switch (CurGameLevelInfo.sSceneName)
								{
									case "SceneForest":
									case "SceneForest 1":
									case "SceneForest_Night":
									case "SceneForest_Dusk":
									case "SceneForest 4":
									case "SceneForest 5":
									case "SceneForest 6":
									case "SceneForest_Rainy":
				...
				bool sceneNeedsNoChest = sceneName == "Yulin_toudan01" || sceneName == "Yulin_toudan02" ||
				                     sceneName == "Map_C" || sceneName == "Map_D";
	...
	public void StartRevive(float fTime)
	{
		...
		if (m_GameUI != null)
		{
			...
			if (m_GameUI.UIManager.mWheelMove != null)
			{
				m_GameUI.UIManager.mWheelMove.gameObject.SetActiveRecursive(false);
			}
			if (m_GameUI.UIManager.mWheelShoot != null)
			{
				m_GameUI.UIManager.mWheelShoot.gameObject.SetActiveRecursive(false);
			}
			if (m_GameUI.UIManager.mWeapon != null)
			{
				m_GameUI.UIManager.mWeapon.gameObject.SetActiveRecursive(false);
			}
			if (m_GameUI.UIManager.mSkill != null)
			{
				m_GameUI.UIManager.mSkill.gameObject.SetActiveRecursive(false);
			}
			if (m_GameUI.UIManager.mFastWeapon != null)
			{
				m_GameUI.UIManager.mFastWeapon.gameObject.SetActiveRecursive(false);
			}

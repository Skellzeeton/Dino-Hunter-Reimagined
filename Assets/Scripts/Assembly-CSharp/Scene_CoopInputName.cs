...
public class Scene_CoopInputName : MonoBehaviour {
    ...
    private void Start() {
        ...
        try {
            label_input.Text = iGameApp.GetInstance().m_GameData.GetDataCenter().NickName;
            if (string.IsNullOrEmpty(label_input.Text))
            {
                DoNewHelp();
            }
            else OnKeyBoardDone(label_input.Text);
        }
        catch {
            DoNewHelp();
            OpenBtnContinue(false);
        }
        player_name = "JohnnyHello";
    ...
    public void TUIEvent_ClickNewHelp(TUIControl control, int event_type, float wparam, float lparam, object data) {
        if (event_type == 3 && popup_new_help != null)
        {
            ...
            {
                popup_new_help.gameObject.SetActiveRecursive(false);
            }
            else
            {
    ...
    public void OpenKeyBoard(string text) {
        IphoneInputPlugin.GetInstance().Open("Enter a name(max 18 letters or numbers)", text, 18, OnKeyBoardDone, TouchScreenKeyboardType.ASCIICapable);
    }
    ...
    protected void OnKeyBoardDone(string sValue) {
        if (sValue.Length >= 2)
        {
            ...
            if (dc.NickName != player_name) {
                dc.NickName = player_name;
                dc.Save();
            }
        }
        else
    ...
    private void UpdateKeyboard() {
        ...
        if (plugin.IsOpen && label_input != null)
        {
            string current = plugin.sValue;
            if (label_input.Text != current)
            {
                label_input.Text = current;
            }
        }
        else if (wasOpen && !plugin.IsOpen && label_input != null)
        {
            if (label_input.Text != player_name)
            {
                label_input.Text = player_name;
            }
        }

...
public class LevelMap : MonoBehaviour
{
	...
	public int FindLevelInArea(int m_level, MapEnterType m_enter_type = MapEnterType.Normal)
	{
		...
		if (m_level >= 10 && m_level <= 11)
		{
			return 3;
		}
		if (m_level >= 12 && m_level <= 15)
		{
			return 4;
		}
		if (m_level >= 16)
		...
		if ((m_level >= 1001 && m_level <= 1010) || (m_level >= 2001 && m_level <= 2010) || (m_level >= 3001 && m_level <= 3010))
		{
			return 1;
		}
		if ((m_level >= 4001 && m_level <= 4010) || (m_level >= 5001 && m_level <= 5010) || ((m_level >= 6001 && m_level <= 6010) || (m_level >= 7001 && m_level <= 7010) || (m_level >= 8001 && m_level <= 8010)) || ((m_level >= 9001 && m_level <= 9010)))
		{
			return 2;
		}
		if ((m_level >= 10001 && m_level <= 10010))
		{
			return 3;
		}
	...
	public void ShowMask(int m_id)
	{
		...
		if (m_id == 5)
		{
			...
			{
				mask_list[i].gameObject.SetActiveRecursive(false);
			}
			return;
		}
		for (int j = 0; j < mask_list.Length; j++)
		{
			if (j == m_id - 1)
			{
				mask_list[j].gameObject.SetActiveRecursive(true);
			}
			else
			{
				mask_list[j].gameObject.SetActiveRecursive(false);
			}
		}
	}
	...
	public void ShowSign(int m_id, Vector3 m_pos, ref Vector3 m_move_pos)
	{
		...
		else if (m_id >= 1 && m_id <= 5)
		{
			for (int i = 0; i < sign_list.Length; i++)
			{
				if (i == m_id - 1)
				{
					sign_list[i].gameObject.SetActiveRecursive(true);
				...
				{
					sign_list[i].gameObject.SetActiveRecursive(false);
				}
			}
		}
		else
		{
			...
			{
				sign_list[j].gameObject.SetActiveRecursive(false);
			}
		}
	}
	...
	public void ShowLevelCoop(bool m_show)
	{
		if (coop_point != null)
		{
			coop_point.gameObject.SetActiveRecursive(m_show);
		}
	}
}

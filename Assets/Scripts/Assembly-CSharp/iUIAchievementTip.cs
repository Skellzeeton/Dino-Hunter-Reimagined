...
public class iUIAchievementTip : MonoBehaviour
{
	...
	private void Update()
	{
		...
		case kState.SlashIn:
			...
			if (!(m_fTimeCount < m_fTime))
			{
				...
				m_fTime = 0.2f;
				m_fTimeCount = 0f;
				mLabel.gameObject.SetActiveRecursive(true);
		...
		case kState.TextIn:
			...
			if (!(m_fTimeCount < m_fTime))
			{
				...
				m_fTime = 0.2f;
				m_fTimeCount = 0f;
				mUIAchievementStar.gameObject.SetActiveRecursive(true);
		...
		case kState.SlashOut:
			...
			if (!(m_fTimeCount < m_fTime))
			{
				...
				m_fTime = 0.2f;
				m_fTimeCount = 0f;
				mUIAchievementStar.gameObject.SetActiveRecursive(false);
				mLabel.gameObject.SetActiveRecursive(false);
	...
	public void ShowTip(string sTip, int nStar)
	{
		...
		mLabel.text = sTip;
		mBackGround.gameObject.SetActiveRecursive(true);

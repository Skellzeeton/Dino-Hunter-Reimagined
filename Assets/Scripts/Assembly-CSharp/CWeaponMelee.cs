...
public class CWeaponMelee : CWeaponBase
{
    ...
    protected void UpdateMelee(CCharPlayer player, float deltaTime)
    {
        ...
        Vector3 rayOrigin = player.GetUpBodyPos() + new Vector3(0f, 0.7f, 0f);
        ...
        foreach (CCharMob value in mobData.Values)
        {
            ...
            Vector3 bloodPos = value.GetBloodPos(rayOrigin, vector2);
            Vector3 dirToBlood = bloodPos - rayOrigin;
            float distToBlood = dirToBlood.magnitude;

            bool losClear = true;

            if (distToBlood > 0.001f)
            {
                RaycastHit hitInfo;
                int layerMask = Physics.DefaultRaycastLayers;
                if (Physics.Raycast(rayOrigin, dirToBlood.normalized, out hitInfo, distToBlood, layerMask, QueryTriggerInteraction.Ignore))
                {
                    CCharMob hitMob = hitInfo.collider.GetComponentInParent<CCharMob>();
                    if (hitMob == null || hitMob != value)
                    {
                        losClear = false;
                    }
                }

...
public class UIDrawCall : MonoBehaviour
{
	...
	public void Set(BetterList<Vector3> verts, BetterList<Vector3> norms, BetterList<Vector4> tans, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (size > 0 && size == uvs.size && size == cols.size && size % 4 == 0)
		{
			...
			mRen.motionVectorGenerationMode = MotionVectorGenerationMode.ForceNoMotion;
			mRen.allowOcclusionWhenDynamic = false;
			mRen.lightProbeUsage = UnityEngine.Rendering.LightProbeUsage.Off;
			mRen.reflectionProbeUsage = UnityEngine.Rendering.ReflectionProbeUsage.Off;
			if (verts.size < 65000)
			{
				int num = (size >> 1) * 3;

...
public class PopupRankingItem : MonoBehaviour
{
	...
	public void SetInfo(RankingType m_type, TUICoopPlayerInfo m_info, GameObject m_invoke_go, ref Dictionary<int, string> m_title_list, ShowBGType m_show_bg = ShowBGType.Keep, bool m_show_infocard = true)
	{
		...
		if (num >= 1 && num <= 3)
		{
			if (img_ranking != null)
			{
				img_ranking.gameObject.SetActiveRecursive(true);
				switch (num)
				{
				case 1:
			...
			{
				label_ranking.gameObject.SetActiveRecursive(false);
			}
		}
		else if (num > 3 && num <= 999)
		{
			if (img_ranking != null)
			{
				img_ranking.gameObject.SetActiveRecursive(false);
			}
			if (label_ranking != null)
			{
				label_ranking.gameObject.SetActiveRecursive(true);
		...
		if (btn_infocard != null)
		{
			...
			btn_infocard.gameObject.SetActiveRecursive(m_show_infocard);
			if (m_show_infocard)
			{
				btn_infocard.Reset();
			}
		...
		if (img_bg != null)
		{
			...
			case ShowBGType.Show:
				img_bg.gameObject.SetActiveRecursive(true);
				break;
			case ShowBGType.UnShow:
				img_bg.gameObject.SetActiveRecursive(false);
				break;
			}
		}
	}

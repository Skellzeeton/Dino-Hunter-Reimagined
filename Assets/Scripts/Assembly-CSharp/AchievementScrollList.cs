    ...
    public void DoCreate(TUIAchievementInfo m_info, GameObject m_go_invoke)
    {
        ...
        for (int i = 0; i < achievement_list.Count; i++)
        {
            ...
            if (achievementItem != null)
            {
				...
				achievementItem.DoCreate(achievement_list[i], (i % 2 != 0) ? true : false, m_go_invoke);
                scrollist.Add(achievementItem.GetComponent<TUIControl>());
                TUIClipBinder component = achievementItem.GetComponent<TUIClipBinder>();
                if (component != null)
                {
                    component.SetClipRect(rect_show);
                    scrollist.Add(component);
                }
            }
            else
            {

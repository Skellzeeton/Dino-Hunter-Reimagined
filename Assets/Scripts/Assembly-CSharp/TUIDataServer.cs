	...
	public class TUIDataServer
	{
		...
		private void TUIEvent_BackInfo_SceneMainMenu(object sender, TUIEvent.SendEvent_SceneMainMenu m_event)
		{
			...
			{
				Application.OpenURL("https://discord.com/invite/4PH6sB3PMs");
			}
			...
			else if (m_event.GetEventName() == TUIEvent.SceneMainMenuEventType.TUIEvent_EnterInfo)
			{
				...
				tUIGameInfo4.villiage_enter_info.finished_text = dataCenter7.SceneProccess + "%";
			...
			else if (m_event.GetEventName() == TUIEvent.SceneEquipEventType.TUIEvent_WeaponSign)
			{
				...
				if (m_GameData.m_WeaponCenter != null)
				{
					...
					foreach (KeyValuePair<int, int> item3 in weaponData)
					{
						...
						tUIWeaponAttribute.crit_chance = cWeaponInfoLevel.fCritical;
						tUIWeaponAttribute.crit_damage = cWeaponInfoLevel.fCriticalDmg;
			...
			else if (m_event.GetEventName() == TUIEvent.SceneStashEventType.TUIEvent_StashInfo)
			{
				...
				if (data != null)
				{
					for (int i = 0; i < data.Count; i++)
					{
						...
						{
							empty = "Expand Stash Capacity to: " + data[i + 1].nCapacity + " ({color:1eff0000}+" + (data[i + 1].nCapacity - data[i].nCapacity) + "{color})";
				...
				if (dataCenter5 != null)
				{
					...
					if (dataCenter5.Crystal < num3)
					{
						global::EventCenter.EventCenter.Instance.Publish(this,
							new TUIEvent.BackEvent_SceneStash(m_event.GetEventName(), false,
								BackEventFalseType.NoCrystalEnough, num3 - dataCenter5.Crystal));
						return;
					}
					...
					iServerSaveData.GetInstance().UploadImmidately();
					global::EventCenter.EventCenter.Instance.Publish(this,
						new TUIEvent.BackEvent_SceneStash(m_event.GetEventName(), true));
				}
			}
		}
		...
		private void TUIEvent_BackInfo_SceneMap(object sender, TUIEvent.SendEvent_SceneMap m_event)
		{
					...
					if (taskInfo != null)
					{
						switch (taskInfo.nType)
						{
							...
							case 4:
								...
								break;
							case 7:
								secondaryLevelType = SecondaryLevelType.Timed;
								break;
						}
					}
			...
			else if (m_event.GetEventName() == TUIEvent.SceneMapEventType.TUIEvent_EnterEndless && m_DataCenter != null)
			{
				if (m_DataCenter.NickName != null && m_DataCenter.NickName.Length > 0)
				{
					global::EventCenter.EventCenter.Instance.Publish(this,
			...
			else if (m_event.GetEventName() == TUIEvent.SceneBlackMarketEventType.TUIEvent_GoodsInfo)
			{
				...
				foreach (KeyValuePair<int, CBlackItem> item in data)
				{
					...
					case 1:
					{
						...
						tUIBlackMarketItem.m_fCritChance = cWeaponInfoLevel.fCritical;
						tUIBlackMarketItem.m_fCritDamage = cWeaponInfoLevel.fCriticalDmg;
						for (int k = 0; k < 3; k++)
						{
		...
		protected void AddWeaponAttribute(TUIWeaponInfo tuiweaponinfo, List<int> ltWeaponList, WeaponType nWeaponType, kShopWeaponCategory nCategory)
		{
			foreach (int ltWeapon in ltWeaponList)
			{
				...
				for (int i = 0; i < weaponInfo.GetLvlCount(); i++)
				{
					...
					tUIWeaponLevelInfo.m_fCritChance = cWeaponInfoLevel2.fCritical;
					tUIWeaponLevelInfo.m_fCritDamage = cWeaponInfoLevel2.fCriticalDmg;
					for (int j = 0; j < 3; j++)
					{

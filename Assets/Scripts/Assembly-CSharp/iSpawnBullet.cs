...
public class iSpawnBullet : MonoBehaviour
{
    ...
    private const float RAY_EPSILON = 0.05f;

    public int Owner
    {
        get { return m_nOwnerUID; }
    }
    ...
    {
        if (target == null || target.isDead || m_HitTargets.Contains(target.UID)) return;
        ...
        {
            if (m_pSkillLvlInfo == null) return;
            for (int i = 0; i < 3; i++)
            {
        ...
        {
            if (m_pWeaponLvlInfo == null) return;
            for (int j = 0; j < 3; j++)
            {
        ...
        if (!actor.IsMonster() && !m_GameScene.IsMyself(actor)) return;
    ...
    protected virtual void OnHitScene(CCharBase actor, Vector3 v3HitPos)
    {
        ...
        foreach (CCharBase item in unitList)
        {
            ...
            float dist = Vector3.Distance(v3HitPos, item.Pos);
            if (dist > fAtkRange) continue;
            if (IsOccludedByObstacle(v3HitPos, item)) continue;
    ...
    protected virtual void OnHitGround(CCharBase actor, Vector3 v3HitPos)
    {
        ...
        foreach (CCharBase item in unitList)
        {
            if (item == null || item.isDead || actor.IsAlly(item)) continue;
            float dist = Vector3.Distance(v3HitPos, item.Pos);
            if (dist > fAtkRange) continue;
            if (IsOccludedByObstacle(v3HitPos, item)) continue;
    ...
    protected virtual void OnHitTarget(CCharBase actor, CCharBase target)
    {
        ...
        if (!m_GameScene.IsMyself(actor) && !actor.IsMonster()) return;
        OnHit(actor, target, m_Transform.position, (target.Pos - m_Transform.position).normalized);

        if (fAtkRange <= 0f) return;
        List<CCharBase> unitList = m_GameScene.GetUnitList();
        if (unitList == null) return;

        foreach (CCharBase item in unitList)
        {
            if (item == null || item.isDead || item == target || actor.IsAlly(item)) continue;
            float dist = Vector3.Distance(m_Transform.position, item.Pos);
            if (dist > fAtkRange) continue;
            if (IsOccludedByObstacle(m_Transform.position, item)) continue;
    ...
    protected virtual void OnTrigger(Collider collider)
    {
        ...
        if (component != null && component.Owner == m_nOwnerUID) return;
    ...
    protected bool IsOccludedByObstacle(Vector3 origin, CCharBase item)
    {
        ...
        try
        {
            targetPoint = item.GetBone(1).position;
        }
        catch
        {
            targetPoint = item.Pos;
        }
        ...
        Vector3 rayOrigin = origin + dir.normalized * RAY_EPSILON;

        RaycastHit hitInfo;
        int layerMask = Physics.DefaultRaycastLayers;

        if (Physics.Raycast(rayOrigin, dir.normalized, out hitInfo, dist - RAY_EPSILON, layerMask, QueryTriggerInteraction.Ignore))
        {
            CCharBase hitChar = hitInfo.collider.GetComponentInParent<CCharBase>();
            if (hitChar == null)
            {
                return true;
            }
            else if (hitChar != item)
            {
                return true;
            }
        }
